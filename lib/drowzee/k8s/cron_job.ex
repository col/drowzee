defmodule Drowzee.K8s.CronJob do
  ...
  def suspend_cron_job(%{"kind" => "CronJob"} = cron_job, suspend) do
    Logger.info("Setting cronjob suspend", cron_job: name(cron_job), suspend: suspend)
    ...
    case K8s.Client.run(Drowzee.K8s.conn(), K8s.Client.update(cron_job)) do
      ...
      {:error, reason} ->
        Logger.error("Failed to suspend cronjob: #{inspect(reason)}", cron_job: name(cron_job), suspend: suspend)
        {:error, reason}
    end
  end
end
